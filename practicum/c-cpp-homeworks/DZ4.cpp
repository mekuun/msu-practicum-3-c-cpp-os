...
#include <cmath>
#include <cstring>
using std::cout, std::cin, std::endl, std::cerr, std::ostream, std::istream;
enum {INCREMENT = 64};

class mstring_exc{
private:
    const char* func_name;
    int param_mode, param1, param2;
public:
    mstring_exc(const char* func_name_inp, int param1_inp):func_name(func_name_inp), param1(param1_inp), param_mode(1){};
    mstring_exc(const char* func_name_inp, int param1_inp, int param2_inp):func_name(func_name_inp), param1(param1_inp), param2(param2_inp), param_mode(2){};
    virtual void print() const;
};


void mstring_exc :: print() const{
    switch(param_mode) {
        case 1:
            cerr << "Exception: problem in" << ' ' << func_name << ' ' << "incorrect parameter:" << param1 << endl;
            break;
        case 2:
            cerr << "Exception: problem in" << ' ' << func_name << ' ' << "incorrect parameter:" << param1 << " " << param2 << endl;
            break;
    }
}
...
2
    ...
    friend mstring operator+(char* str, const mstring& op);
...
void mstring::insert(char c, int i){
    if (i < 0 || i > strlen(string)) {throw(mstring_exc("insert", i));}
...
void mstring::del(int i){
    if (i < 0 || i >= strlen(string)) throw(mstring_exc("del", i));
...
void mstring::del(int i, int j){
    if (i < 0 || i >= strlen(string) || j >= strlen(string) || j < i) throw(mstring_exc("del", i, j));
    char *temp = new char[strlen(string) + 1];
    strncpy(temp, string, i);
    temp[i] = '\0';
...
mstring operator+(char* str, const mstring& op){
    mstring tmp = str;
    tmp.add(op.string);
    return tmp;
}
...
{
    if (x < 0) throw(mstring_exc("operator*", x));
    mstring ans;
    if (x == 0) return ans;
...
char& mstring :: operator[](int index){
    if (index < 0 || index >= strlen(string)) throw(mstring_exc("operator[]", index));
